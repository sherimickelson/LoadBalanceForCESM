...
class IceLndAtmOcn(optimize_model.OptimizeModel):
    ...
    def optimize(self):
        ...
        if(self.optimizer == 1):
            self.prob.solve()  #Coin-CBC
        elif(self.optimizer ==2):
            self.prob.solve(pulp.GLPK()) # GLPK()
...
class IceLndWavAtmOcn(optimize_model.OptimizeModel):
    ...
    def optimize(self):
        ...
        # cost function 
        self.prob += X['TotalTime'],"obj"
        
        #constraints
        self.constraints = []
        # Layout-dependent constraints. Choosing another layout to model
        # will require editing these constraints
        # changed the names of the constraits because GLPK was having trouble with them - SK-July 2020
        self.constraints.append([X['Tice'] - X['T1'] <= 0, "C1"]) #"Tice - T1 == 0"])
        self.constraints.append([X['Tlnd'] - X['T1'] <= 0, "C2"]) #"Tlnd - T1 == 0"])
        self.constraints.append([X['Twav'] - X['T1'] <= 0, "C3"]) #"Twav - T1 == 0"])
        self.constraints.append([X['T1'] + X['Tatm'] - X['TotalTime'] <= 0,"C4"]) #  "T1 + Tatm - TotalTime <= 0"])
        self.constraints.append([X['Tocn'] - X['TotalTime'] <= 0, "C5"]) #  "Tocn - TotalTime == 0"])
        self.constraints.append([X['Nice'] + X['Nlnd'] + X['Nwav'] - X['Natm'] == 0,"C6"]) #  "Nice + Nlnd + Nwav - Natm == 0"])
        self.constraints.append([X['Natm'] + X['Nocn'] == self.maxtasks,"C7"]) # "Natm + Nocn <= %d" % (self.maxtasks)]), changed from equality to inequality
        self.constraints.append([self.atm.blocksize * X['NBatm'] - X['Natm'] == 0,"C8"]) # "Natm = %d * NBatm" % self.atm.blocksize])
        self.constraints.append([self.ice.blocksize * X['NBice'] - X['Nice'] == 0, "C9"]) # "Nice = %d * NBice" % self.ice.blocksize])
        self.constraints.append([self.lnd.blocksize * X['NBlnd'] - X['Nlnd'] == 0,"C10"]) # "Nlnd = %d * NBlnd" % self.lnd.blocksize])
        self.constraints.append([self.ocn.blocksize * X['NBocn'] - X['Nocn'] == 0, "C11"]) # "Nocn = %d * NBocn" % self.ocn.blocksize])
        self.constraints.append([self.wav.blocksize * X['NBwav'] - X['Nwav'] == 0,"C12"]) # "Nwav = %d * NBwav" % self.wav.blocksize])
        print 'check', self.ice.blocksize, self.atm.blocksize 
        # These are the constraints based on the timing data.
        # They should be the same no matter what the layout of the components.
        self.add_model_constraints()
...
#Updated this so that we can use the CVXOPT -SK July 2020
    def get_solution(self):
        """
        Return a dictionary of the solution variables.
        """
