...
"""
#import timeit  # used for timing the optimizers
#import time    # used for timing the optimizers
import re
import json
try:
...
#-------------------SK added this- July-2020-------------------------------------------------
    parser.add_argument('--optimizer', type=int,
                        help='chose between the two Mixed Integer Linear Solvers  1-PuLP_COIN-CBC, 2-PuLP_GLPK')
#------------------------------------------------------------------------------------
...
################################################################################
def load_balancing_solve(test_id, test_root, timing_dir, blocksizes, total_tasks,optimizer, layout, graph_models, print_models, pe_output, json_output, json_input):
################################################################################
    if json_input is not None:
        ...
        #added this to pass the optimizer choice to the optimiz and layout modules
        data['optimizer_choice'] = optimizer
        if layout is None:
    ...
    if(optimizer == 1):
        ...
        logger.info("Solving Mixed Integer Linear Program using CVXOPT interface to GLPK")
    else:
        expect(False, "Could not automatically determin the optimizer")

#    start_time = time.time()  # used to time the optimizers
    status = opt.optimize()
#    end_time = time.time()
#    total_time = end_time - start_time
#    print '****************', total_time
    if(optimizer == 1):
        logger.info("PuLP-COIN-CBC solver status: " + opt.get_state_string(status))
    elif(optimizer == 2):
        logger.info("PuLP-GLPK solver status: " + opt.get_state_string(status))
...
###############################################################################
# added optimizer to both of the lines below- July 2020
    test_id, test_root, timing_dir, blocksizes, total_tasks,optimizer, layout, graph_models, print_models, pe_output, json_output, json_input = parse_command_line(sys.argv, description)

    sys.exit(load_balancing_solve(test_id, test_root, timing_dir,blocksizes, total_tasks,optimizer, layout, graph_models, print_models, pe_output, json_output, json_input))
###############################################################################
